...
function hasPermission(permission: Permission) {
  const isSnowProtocol = isProtocol("snow:");
  const isSnowInternalProtocol = isProtocol("snow-internal:");

  const isInternalProtocols = isSnowInternalProtocol || isSnowProtocol;

  // Browser UI
  const isMainUI = isLocation("snow-internal:", "main-ui");
  const isPopupUI = isLocation("snow-internal:", "popup-ui");
  const isBrowserUI = isMainUI || isPopupUI;

  // Windows
  const isNewTab = isLocation("snow:", "new-tab");
  const isOmniboxUI = isLocation("snow-internal:", "omnibox");
  const isOmniboxDebug = isLocation("snow:", "omnibox");
  const isOmnibox = isOmniboxUI || isNewTab || isOmniboxDebug;

  // Extensions
  const isExtensions = isLocation("snow:", "extensions");
  ...
  switch (permission) {
    ...
    case "session":
      return isSnowInternalProtocol || isOmnibox || isBrowserUI;
    case "settings":
