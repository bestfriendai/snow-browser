...
const SNOW_PROTOCOL_ALLOWED_DOMAINS: AllowedDomains = {
  about: true,
  error: true,
  "new-tab": true,
  games: true,
...
export function registerSnowProtocol(protocol: Protocol) {
  const handleDomainRequest = async (request: Request, url: URL) => {
    const hostname = url.hostname;
    const pathname = url.pathname;

    if (!(hostname in SNOW_PROTOCOL_ALLOWED_DOMAINS)) {
      return new Response("Invalid request path", { status: 400 });
    }

    const allowedPath = SNOW_PROTOCOL_ALLOWED_DOMAINS[hostname];
  ...
  protocol.handle("snow", async (request) => {
