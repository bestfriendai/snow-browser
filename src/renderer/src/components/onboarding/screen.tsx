...
export function OnboardingScreen({ children }: { children?: React.ReactNode }) {
  ...
  const createShape = useCallback((): ShapeData | null => {
    ...
    // Generate color in clean red range with some base opacity
    const redValue = Math.floor(220 + Math.random() * 35); // 220-255 (brighter reds)
    const greenValue = Math.floor(80 + Math.random() * 60); // 80-140 (less green for cleaner red)
    const blueValue = Math.floor(80 + Math.random() * 60); // 80-140 (less blue for cleaner red)
    const baseOpacity = 0.2 + Math.random() * 0.15; // 0.2-0.35 (slightly more subtle)
    const color = `rgba(${redValue}, ${greenValue}, ${blueValue}, ${baseOpacity.toFixed(2)})`;
  ...
  // --- Render Component ---
  return (
    <div className="app-drag select-none relative h-screen w-full overflow-hidden red-gradient-bg flex flex-col items-center justify-center">
      {/* Set document title */}
      <title>Onboarding | Snow Browser</title>
      ...
      <motion.div
        ...
        className="absolute top-[-20%] left-[-10%] w-[70%] h-[70%] rounded-full bg-radial from-white/10 via-gray-300/5 to-transparent blur-[60px] z-[5] pointer-events-none"
        aria-hidden="true"
      />
      ...
      <motion.div
        ...
        className="absolute bottom-[-30%] right-[-20%] w-[80%] h-[80%] rounded-full bg-radial from-white/8 via-gray-400/4 to-transparent blur-[70px] z-[5] pointer-events-none"
        aria-hidden="true"
      />
