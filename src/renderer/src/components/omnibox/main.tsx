...
export function OmniboxMain() {
  ...
  return (
    ...
    >
      <motion.div
        className="w-full h-full mx-auto flex flex-col min-h-0"
        initial={{ opacity: 0 }}
      ...
      >
        <Command
          className={cn(
            ...
            matches.length > 0 ? "h-full" : "max-h-full"
          )}
          loop
          value={selectedValue}
        ...
        >
          {/* Logo Section */}
          <div className="flex justify-center py-6 border-b border-white/10 flex-shrink-0">
            <img
          ...
          <div className="flex items-center p-3.5 border-b border-black/10 dark:border-white/10 flex-shrink-0">
            <CommandInput
              ...
              onBlur={handleBlur}
              ref={inputRef}
              className="size-full outline-none text-lg font-medium text-white placeholder:text-white/60"
            />
          </div>

          {matches.length > 0 && (
            <CommandList
              className="flex-1 px-1.5 py-2 overflow-y-auto min-h-0"
              style={{
                scrollbarWidth: "thin",
